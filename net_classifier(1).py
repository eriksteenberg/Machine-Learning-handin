...
def softmax(X):
    ...
    ### YOUR CODE HERE
    XA=np.amax(X,axis=1)[:, np.newaxis]
    res = np.exp(X-XA-np.log(np.sum(np.exp(X-XA),axis=1))[:, np.newaxis])
    ### END CODE
    return res
...
class NetClassifier():
    ...
    def predict(self, X, params=None):
        ...
        W2 = params['W2']
        b2 = params['b2']
        out = softmax((relu(X@W1+b1))@W2+b2)
    ...
    def fit(self, X_train, y_train, X_val, y_val, init_params, batch_size=32, lr=0.1, c=1e-4, epochs=30):
        ...
        for i in range(epochs):
            ...
            for j in range(0,len(Xy_train),batch_size):
                
                XyB=Xy_train[j:j+batch_size,:]
                XB=XyB[:,:-1]
                yB=XyB[:,-1]
                
                W = self.cost_grad(XB,yB,{'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2},c)[1]
                W1 = W1 - lr*W['d_w1']
                b1 = b1 - lr*W['d_b1']
                W2 = W2 - lr*W['d_w2']
                b2 = b2 - lr*W['d_b2']
                
            train_loss.append(self.cost_grad(X_train,y_train,{'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2},c)[0])
            val_loss.append(self.cost_grad(X_val,y_val,{'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2},c)[0])
            train_acc.append(self.score(X_train,y_train,{'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}))
            val_acc.append(self.score(X_val,y_val,{'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}))
        ### END CODE
